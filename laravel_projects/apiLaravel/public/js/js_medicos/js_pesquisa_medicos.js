	    ...
	    function busca(volta_primeira_pagina=0){ 
	        ...
	        $.ajax({    
	                ...
	                success:function(results){
				        ...
				        $.each(results.lista, function() {
                            ...
                            if(this.status==0){
                            var excluir = "<a href='#' alt='Excluir Médico' onclick='exclui_medico("+this.id+")'  ><i class='espaco_icone_acao fa fa-trash-o'></i></a>";
                            }else{
                                var excluir = "";
                            }
				            ...
				            $("#listagem_dados").append("<tr>"+
				                                            "<td>"+excluir+"<a href='./cadastrar_medico?id_registro="+this.id+"' ><i class='espaco_icone_acao fa fa-pencil'></i></a>"+
				                                            "</td>"+
				                                            "<td>"+this.nome+"</td>"+
				                                            "<td>"+this.crm+"</td>"+
				                                            "<td>"+this.telefone+"</td>"+
				                                            "<td>"+this.lista_especialidades+"</td>"+
				                                        "</tr>");
				        });
            ...
            function exclui_medico(id_medico){

                 // abre a confirmacao para cancelar a PV
                    if (confirm("Deseja confirmar a exclusão do médico ?")) {
                      envia_exclusao(id_medico)
                    }
   ...
   /**********Funcao que envia o post para o controller que informa PEP************/
      function envia_exclusao(id_medico){
