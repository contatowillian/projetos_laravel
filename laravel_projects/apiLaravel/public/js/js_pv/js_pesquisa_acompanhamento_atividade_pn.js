      ...
      //chama a busca assim que inicia * nesta pagina nao é ultilizado
			//busca();
    ...
    /* Funcao que gerencias a opcao de colunas do relatorio*/

    function gerencia_colunas(){

  
      $('#opcoes_coluna input:checkbox').each(function () {
         
         var opcao_coluna = $(this).val();     
         $('.'+opcao_coluna).hide();
      });

      $('#opcoes_coluna input:checkbox:checked').each(function () {
    ...
    function pega_lista_aprovacoes_pv(id_pv_capex){
        ...
        $.ajax({
                type: 'POST',  
                url: './pega_lista_aprovacoes_pv_status_controle',
                contentType: "application/x-www-form-urlencoded;charset=ISO-8859-15",
        ...
        //verifica se o usuario escolheu pelo menos 1 coluna pra mostrar 
        var coluna_escolhida = 0;
        $('#opcoes_coluna input:checkbox:checked').each(function () {
           coluna_escolhida = 1;
        })  
        ...
        if(coluna_escolhida==0){
          ...
          alert('Escolha pelo menos 1 coluna para efetuar a busca!');
          return false;
        }
        ...
        var  sessao_expirada = valida_sessao_expirada();
	        ...
	        $.ajax({    
                ...
                type: 'POST',  
                url: './lista_acompanhamento_atividade',  
                ...
                if(!this.id_escopo_projeto){
                  this.id_escopo_projeto='';
                }
                ...
                if(!this.valor_bruto_formatado){
                  this.valor_bruto_formatado='';
                }
                ...
                if(!this.data_insercao){
                 ...
                 if(!this.data_criacao_pedido){
                  this.data_criacao_pedido='';
                }
                ...
                if(!this.data_ultima_alteracao_status){
                  this.data_ultima_alteracao_status='';
                }

                if(!this.usuario_alteracao_controle){
                  this.usuario_alteracao_controle='';
                }

                if(!this.id_solicitacao_capex){
                  this.id_solicitacao_capex='';
                }
                ...
                if(this.status_plano_nominal=='1'){
                  var imagem ='<img class="icone_status_controle" src="imagens/ball_red.png" alt="Plano nominal Inativo"> ';
                  var css_cor_linha= 'css_cor_linha_vermelha';
                }else if(this.status_plano_nominal=='2'){
                  var imagem ='<img class="icone_status_controle" src="imagens/ball_green.png" alt="Plano Alterado"> ';
                  var css_cor_linha= 'css_cor_linha_amarela';
                }else{  
                  var imagem ='<img class="icone_status_controle" src="imagens/ball_green.png" alt="Plano Ativo"> ';
                  var css_cor_linha= 'css_cor_linha_verde';
                }

                // imagem nao sera utilizada
                var imagem ='';

              
                //pega_lista_aprovacoes_pv(this.id_solicitacao_capex);
                var html_lista = "<tr class='"+css_cor_linha+"'><td class='coluna_capex acao_controler_capex' ><i onclick='abre_modal("+this.id_solicitacao_capex+",\"detalhe_pv\")' data-target='#modal_detalhe_pv' data-toggle='modal'   class='espaco_icone_acao_aprovacao fa fa-search-plus ' title='Exibir detalhes da PV'></i>"+
                                              ...
                                              imagem+"</td>"+
                                              "<td class='coluna_capex'>"+this.CAPEX+"</td>"+
                                              "<td class='coluna_capex'>"+this.PEP_Nivel_2+"</td>"+
                                              "<td class='coluna_capex'>"+this.site_capex+"</td>"+
                                              "<td class='coluna_informacoes_pn'>"+this.REGIONAL+"</td>"+
                                              "<td class='coluna_informacoes_pn'>"+this.SITE+"</td>"+
                                              "<td class='coluna_informacoes_pn'>"+this.ATIVIDADE+"</td>"+
                                              "<td class='coluna_informacoes_pn'>"+this.nome_fornecedor+"</td>"+
                                              "<td class='coluna_informacoes_pn'></td>"+
                                              "<td class='coluna_informacoes_pn'>"+this.status_estacao_bdraf+"</td>"+
                                              "<td class='coluna_informacoes_pn'></td>"+
                                              "<td class='coluna_solicitacao_verba'>"+this.id_solicitacao_capex+"</td>"+
                                              "<td class='coluna_solicitacao_verba' >"+this.escopo_projeto+"</td>"+
                                              "<td class='coluna_solicitacao_verba tamanho_nome_pv' >"+this.nome_pv+"</td>"+
                                              "<td class='coluna_solicitacao_verba' >"+this.valor_bruto_formatado+"</td>"+
                                              "<td class='coluna_solicitacao_verba' >"+this.data_insercao+"</td>";

                  //monta a lista de aprovacoes para atividades sem solicitacoes                            
                  if(!this.id_solicitacao_capex || this.id_solicitacao_capex==''){ 
                          $.each(results.atividade_sem_solicitacao.retorno_lista_aprovacoes.retorno_aprovacao, function(key2,value2) {
                              html_lista += "<td class='coluna_aprovacoes'>"+value2.data_aprovacao+"</td>";
                          });
                  }else{

                          var id_solicita = this.id_solicitacao_capex;
                          $.each(results.aprovacoes[id_solicita].aprovacoes, function(key2,value2) {
                              html_lista += "<td class='coluna_aprovacoes'>"+this.data_aprovacao+"</td>";
                          });
                  }
             
                  if(this.id_solicitacao_capex && this.id_solicitacao_capex!='' &&
                     this.id_plano_nominal_sites && this.id_plano_nominal_sites!='' 
                    ){ 

                    var id_solicita = this.id_solicitacao_capex;
                    var id_pns = this.id_plano_nominal_sites;
                    var html_select = results.html_select_status_controle[id_solicita][id_pns] ;

                  }else{
                    var html_select = ""; 
                  }
                  
                 html_lista += "<td class='coluna_pedido'>"+this.pedido+"</td>"+
                                    "<td class='coluna_pedido'>"+this.item+"</td>"+
                                    "<td class='coluna_pedido'>"+this.data_criacao_pedido+"</td>"+
                                    "<td class='coluna_status'>"+html_select+"</td>"+
                                    "<td class='coluna_status'>"+this.data_ultima_alteracao_status+"</td>"+
                                    "<td class='coluna_status'>"+this.usuario_alteracao_controle+"</td>"+
                               "</tr>";

				        $("#listagem_dados").append(html_lista);
                 ...
                 gerencia_colunas();
                	...
                	$("#numero_registros").html('');
            			...
            			var quantidade_inicial = numero_registro_pagina*$( "#limite option:selected" ).val();
            				...
            				$("#listagem_dados").html( "<tr align='center' ><td colspan='60'><i class='fa fa-frown-o  fa_nao_encontrado'></i>  Nenhum registro encontrado com o filtro selecionado</td></tr>" );
    ...
    //muda o status do controle
    function muda_status_controle(status_controle,id_pv_capex,id_plano_nominal_sites){
        ...
        $.ajax({
                type: 'POST',  
                url: './muda_status_controle',
                contentType: "application/x-www-form-urlencoded;charset=ISO-8859-15",
                data : {id_status_controle: status_controle , id_capex : id_pv_capex, plano_nominal_sites : id_plano_nominal_sites  }, 
                dataType:'json',
                ...
                success:function(data){
                        ...
                        if(data[0]===true){
                           ...
                           alert("Erro ao alterar status, informe ao administrador do sistema!");
                           //carrega o grid novamente
                           return false; 
                        }
                },
