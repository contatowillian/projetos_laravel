                ...
                success:function(data){
                        ...
                        if(data[0]===true){
                            ...
                            $.each(data.detalhes, function(key,value) {
                              ...
                              if(this.id_perfil_acesso_aprovador_atual==21){
                                $("#box_aprovacao_analista").show();
                                $( "#form_detalhe_pv_pep2" ).show();
                              }else{
                                $("#box_aprovacao_analista").hide();
                                $( "#form_detalhe_pv_pep2" ).hide();
                              }
        ...
        var  sessao_expirada = valida_sessao_expirada();
	        ...
	        $.ajax({    
                ...
                */
                ...
                //se for o analista que for aprovador tem que ser enviando arquivo o checkbox é bloqueado
                if(this.id_perfil_acesso_aprovador_atual==21){
                  var input_aprovacao  = "<i class='espaco_icone_acao_aprovacao fa fa-ban icone_sem_aprovadores ' title='A aprovacao do Analista é feita após envio de PV nos detalhes do arquivo'></i>";
                  var reprovar_pv  = "";

                //verifica se existe aprovacao pra a PV, caso nao tem ele mostra a info
                }else if(this.max_index_aprovacao==0 || this.max_index_aprovacao=='' || this.max_index_aprovacao===null || !this.max_index_aprovacao){
                  var input_aprovacao  = "<i class='espaco_icone_acao_aprovacao fa fa-ban icone_sem_aprovadores ' title='Não existe aprovadores para esta pv'></i>";
                  var reprovar_pv  = "";
                
                }else if(this.cod_status_capex==0){
                  var input_aprovacao  = "<input type='checkbox' value='"+this.id_solicitacao_capex+"' id='id_capex_aprovacao_"+this.id_solicitacao_capex+"' name='id_capex_aprovacao_"+this.id_solicitacao_capex+"'/>";
                  var reprovar_pv  = "<i onclick='reprovar_pv("+this.id_solicitacao_capex+")' class='espaco_icone_acao_aprovacao fa fa-hand-paper-o' ></i>";
                  ...
                  var reprovar_pv  = "";
				        ...
				        $("#listagem_dados").append("<tr>"+
                                              "<td >"+input_aprovacao+                       
                                              "<i onclick='abre_modal("+this.id_solicitacao_capex+",\"detalhe_pv\")' data-target='#modal_detalhe_pv' data-toggle='modal'   class='espaco_icone_acao_aprovacao fa fa-search-plus ' title='Exibir detalhes da PV'></i>"+
                                              ...
                                              reprovar_pv+
                                              "</td>"+
                                              "<td>"+this.data_solicitacao+"</td>"+
                                              "<td>"+this.id_solicitacao_capex+"</td>"+
                                              "<td>"+this.macro_projeto+"</td>"+
                                              "<td>"+this.site+"</td>"+
                                              "<td>"+this.lista_atividades+"</td>"+
                                              "<td>"+this.fornecedor+"</td>"+
                                              "<td  align='center'>"+this.etapa_atual+"</td>"+
                                              "<td  align='center'>"+this.status_pv+"</td>"+
                                         "</tr>");
				        });
